#!/usr/bin/env python3
"""P16: MIN_EXPECTED_PNL_PCT=-1.0% (was -0.5%), rest = P14-V1"""
import sys, os, time
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

import backtester
backtester.MIN_PNL_CHECK_H = 72
backtester.MIN_EXPECTED_PNL_PCT = -1.0   # P16: was -0.5%
...
print(f"P16: MIN_EXPECTED_PNL=-1.0% (was -0.5%) | {DAYS} days, 4H, {len(SYMBOLS)} coins, $1000, risk=2%")
print(f"Period: {START.date()} -> {END.date()}")
print(f"Baseline P14-V1: Trades 96 | WR 68.8% | PnL +$37.84 (+3.78%) | PF 1.08\n")
...
print(f"\n{'='*100}")
print(f"  P16: MIN_EXPECTED_PNL=-1.0%, MIN_PNL_TIMEOUT=72h, $1000, risk=2%")
print(f"{'='*100}")
...
# Compare MIN_PNL_TIMEOUT with P14-V1
timeout_trades = [x for x in all_trades if getattr(x, 'exit_reason', '') == 'MIN_PNL_TIMEOUT']
timeout_pnl = sum(x.pnl_usd for x in timeout_trades)
print(f"\nMIN_PNL_TIMEOUT comparison:")
print(f"  P14-V1 (threshold=-0.5%): 13 trades, PnL=${-128.20:+.2f}")
print(f"  P16   (threshold=-1.0%): {len(timeout_trades)} trades, PnL=${timeout_pnl:+.2f}")
saved = 13 - len(timeout_trades)
print(f"  Trades saved from timeout: {saved} ({13} -> {len(timeout_trades)})")
