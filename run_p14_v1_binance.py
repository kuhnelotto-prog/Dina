#!/usr/bin/env python3
"""P14-V1 baseline on Binance data (same data source as P16 for fair comparison)"""
import sys, os, time
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

import backtester
backtester.MIN_PNL_CHECK_H = 72
backtester.MIN_EXPECTED_PNL_PCT = -0.5   # P14-V1 original
...
print(f"P14-V1-BINANCE (control) | {DAYS} days, 4H, {len(SYMBOLS)} coins, $1000, risk=2%")
print(f"MIN_EXPECTED_PNL=-0.5%, MIN_PNL_TIMEOUT=72h (same as original P14-V1)")
print(f"Period: {START.date()} -> {END.date()}")
print(f"Data source: Binance USDT-Futures\n")
...
print(f"\n{'='*100}")
print(f"  P14-V1-BINANCE: MIN_EXPECTED_PNL=-0.5%, MIN_PNL_TIMEOUT=72h, $1000, risk=2%")
print(f"{'='*100}")
