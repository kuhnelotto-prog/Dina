#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P40 Fine-tune around SL=6.5: 6.2, 6.4, 6.6, 6.8, 7.5."""
import sys, os, time
...
SWEEP_VALUES = [6.2, 6.4, 6.6, 6.8, 7.5]
print("Fetching data...")
data_cache = {}
...
# COMPLETE TABLE with known context
print(f"\n{'='*90}")
print(f"  FINE-TUNE AROUND SL=6.5 (with context)")
print(f"{'='*90}")
print(f"  {'SL':>3s} | {'BULL_PnL':>9s} {'SL%':>5s} | {'BEAR_PnL':>9s} {'SL%':>5s} | {'CUR_PnL':>8s} {'SL%':>5s} | {'GRAND':>8s}")
print(f"  " + "-" * 70)
context = [
    (5.5, -58.33, 6.7, 254.01, 5.1, 333.33, 2.8),
    (6.0, -15.42, 5.0, 276.61, 3.2, 331.23, 2.8),
    (6.5, 35.79, 3.7, 279.10, 3.2, 329.44, 2.8),
    (7.0, 9.04, 3.3, 283.91, 2.8, 329.06, 1.4),
]
for sl, bp, bsl, bep, besl, cp, csl in context:
    grand = bp + bep + cp
    marker = " <--- BEST" if sl == 6.5 else ""
...
for sl_val in SWEEP_VALUES:
    ...
    grand = bp + bep + cp
    marker = " <--- NEW BEST?" if grand > 644.32 else ""
    print(f"  {sl_val:>3.1f} | {bp:>+9.2f} {bsl:>4.1f}% | {bep:>+9.2f} {besl:>4.1f}% | {cp:>+8.2f} {csl:>4.1f}% | {grand:>+8.2f}{marker}")
