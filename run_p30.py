#!/usr/bin/env python3
"""P30: BTC Health scoring (replaces binary 1D EMA50 filter).

btc_health = 0.0
  +0.35 if btc_close > ema50_1d else -0.35
  +0.35 if ema50_1d_slope > 0 else -0.35
  +0.15 if btc_close > ema200_1d else -0.15
  +0.15 if ema50_1d > ema200_1d else -0.15

btc_1d_allows_long = (btc_health >= 0.40)
btc_1d_allows_short = (btc_health <= -0.40)
"""
import sys, os, time
...
BALANCE = 1000.0
WARMUP_DAYS = 250  # extra days for EMA200 warmup
...
# ── Fetch ALL data once (with 1D EMA200 warmup) ──
print("Fetching data for all periods (with 250-day 1D warmup for EMA200)...")
data_cache = {}
...
# ── Run P30 ──
print(f"\n{'='*90}")
print(f"  P30: BTC Health scoring (threshold=0.40) vs P24 baseline")
print(f"  health = 0.35*(close>ema50) + 0.35*(slope>0) + 0.15*(close>ema200) + 0.15*(ema50>ema200)")
print(f"  LONG requires health >= 0.40, SHORT requires health <= -0.40")
print(f"{'='*90}")
