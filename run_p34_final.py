#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P34 final test: SL_ATR_MULT_LONG=6.6 (from p39/p40 sweep), matching sweep params."""
import sys, os, time
sys.path.insert(0, '.')

import backtester as bt_mod

# P34 parameters (from sweep optimal)
bt_mod.SL_ATR_MULT_LONG = 6.6    # OPTIMAL from p39/p40 sweep
bt_mod.SL_ATR_MULT_SHORT = 1.5    # standard SL for shorts
bt_mod.TSL_ATR_LONG_AFTER_TP1 = 2.0  # softer trailing after TP1 for LONG

# Match sweep thresholds
bt_mod.THRESHOLD_LONG_BULL = 0.40
bt_mod.THRESHOLD_LONG_BEAR = 0.45
bt_mod.THRESHOLD_SHORT_BULL = 0.45
bt_mod.THRESHOLD_SHORT_BEAR = 0.35
bt_mod.BASE_RISK_PCT = 2.0  # 2% risk as in sweep

SYMBOLS_12 = ["BTCUSDT","ETHUSDT","BNBUSDT","XRPUSDT","SOLUSDT","LINKUSDT","DOGEUSDT","AVAXUSDT","ADAUSDT","SUIUSDT","APEUSDT","ARBUSDT"]
...
print("=" * 80)
print("P34 FINAL: SL_ATR_MULT_LONG=6.6 (optimal from sweep)")
print(f"Thresholds: LONG_BULL=0.40, LONG_BEAR=0.45, SHORT_BULL=0.45, SHORT_BEAR=0.35")
print(f"Risk=2%, START=${BALANCE:,.0f}, SYMBOLS=12")
print("=" * 80)
...
for pk, start_dt, end_dt in PERIODS:
    ...
    print(f"\n  {pk:12s} LONG: {len(long_trades):3d} WR={long_wr:.1f}% PnL=${long_pnl:+.2f} SL={long_sl_pct:.1f}%  |  SHORT: {len(short_trades):3d} WR={short_wr:.1f}% PnL=${short_pnl:+.2f}  |  TOTAL: ${total_pnl:+.2f}  DD={max_dd/BALANCE*100:.1f}%")
...
print(f"\n{'='*80}")
print(f"  GRAND TOTAL (P34 FINAL, SL_LONG=6.6): ${grand_pnl:+.2f}  MaxDD={grand_dd:.2f}$")
print(f"{'='*80}")