...
def run_one_period(pk, dfs, btc_df, btc_1d):
    all_trades = []
    for s in SYMBOLS_12:
        sym_df = dfs.get(s)
        if sym_df is None or sym_df.empty or len(sym_df) < 50:
            continue
        # Pass only current symbol + BTC (matches run_p24.py)
    ...
    long_step = Counter(getattr(x, 'trailing_step', 0) for x in long_t)
    short_step = Counter(getattr(x, 'trailing_step', 0) for x in short_t)
...
print(f"\n{'='*90}")
print(f"  P34: LONG=SL3.5+TP1/TP2+TSL | SHORT=SL1.5+TRAILING_STAGES(unchanged)")
print(f"{'='*90}")

for pk, start_dt, end_dt in PERIODS:
    dfs, btc_df, btc_1d = data_cache[pk]
    r = run_one_period(pk, dfs, btc_df, btc_1d)
    if r is None:
        print(f"\n  {pk}: NO TRADES"); continue
    print(f"\n  {pk}")
    ...
    print(f"  TOTAL: {r['total']:>3} trades | PnL=${r['pnl']:+.2f}")
    print(f"\n  --- Exit Reasons ---")
    for reason in sorted(set(list(r['long_reasons'].keys()) + list(r['short_reasons'].keys()))):
        l_c = r['long_reasons'].get(reason, 0); s_c = r['short_reasons'].get(reason, 0)
        print(f"  {reason:20s}: LONG={l_c:3d} | SHORT={s_c:3d}")
    print(f"\n  --- SL Hit Rate ---")
    print(f"  LONG  SL-hit: {r['long_sl_rate']:.1f}%")
    print(f"  SHORT SL-hit: {r['short_sl_rate']:.1f}%")
    print(f"\n  --- Step Distribution ---")
    all_steps = sorted(set(list(r['long_step'].keys()) + list(r['short_step'].keys())))
    for step in all_steps:
        l_c = r['long_step'].get(step, 0); s_c = r['short_step'].get(step, 0)
        print(f"  Step {step}: LONG={l_c:3d} | SHORT={s_c:3d}")
