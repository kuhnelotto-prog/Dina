...
class TradeLog:
    ...
    async def init(self):
        ...
        # P38: add signals_fired and composite_score columns if they don't exist (for existing DBs)
        try:
            await self._conn.execute("ALTER TABLE trades ADD COLUMN signals_fired TEXT")
        except Exception:
            pass  # column already exists
        try:
            await self._conn.execute("ALTER TABLE trades ADD COLUMN composite_score REAL DEFAULT 0.0")
        except Exception:
            pass  # column already exists
    ...
    async def record_entry(self, trade: TradeRecord):
        ...
        await self._conn.execute("""
            INSERT INTO trades 
            (trade_id, symbol, side, size_usd, entry_price, entry_time, bot_id, tags, source, commission, commission_asset, signals_fired, composite_score)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            trade.trade_id,
            trade.symbol,
            ...
            json.dumps(trade.signals_fired) if trade.signals_fired else None,
