...
# When USE_P34_LONG_EXIT=True: LONG uses TP1/TP2 + TSL from peak
# When USE_P34_LONG_EXIT=False (default): LONG uses TRAILING_STAGES (P24 baseline)
SL_ATR_MULT_LONG = 1.5       # default = same as P24; P34 scripts override to 3.5
USE_P34_LONG_EXIT = False     # default = P24 baseline
TP1_ATR_MULT = 1.0
TP1_CLOSE_PCT = 0.30
TP2_ATR_MULT = 2.0
TP2_CLOSE_PCT = 0.30
TSL_ATR_LONG = 2.0           # P34: TSL distance from peak for longs
...
class BacktestPosition:
    ...
    """
    def __init__(self, symbol, side, entry_price, size_usd, sl_price, tp_price, timestamp, entry_atr=0.0, composite_score=0.0):
        self.symbol = symbol
        self.side = side
        ...
        # P34: composite score + peak/valley for TSL tracking
        self.composite_score = composite_score
        self._peak_price = entry_price   # highest price seen (for long TSL)
        self._valley_price = entry_price  # lowest price seen (for short TSL)
    ...
    def update(self, current_price, high=None, low=None, timestamp=None):
        ...
        # ── Update peak/valley for P34 TSL ──
        if self.side == "long":
            self._peak_price = max(self._peak_price, candle_high)
        else:
            self._valley_price = min(self._valley_price, candle_low)

        # ── Exit logic: P34 for LONG (when enabled), TRAILING_STAGES otherwise ──
        if self.side == "long" and USE_P34_LONG_EXIT:
            closed = self._apply_long_exit(current_price)
        else:
    ...
    def _apply_long_exit(self, close):
        """P34 LONG: TP1/TP2 + TSL from peak."""
        ATR = self.entry_atr
        if ATR <= 0:
            return False
        atr_move = (close - self.entry_price) / ATR
        # TP1: +1.0xATR -> close 30%, SL to breakeven
        if self.trailing_step == 0 and atr_move >= TP1_ATR_MULT:
            self.trailing_step = 1
            if TP1_CLOSE_PCT > 0 and self.remaining_pct > 0:
                pct_of_current = TP1_CLOSE_PCT / self.remaining_pct
                pct_of_current = min(pct_of_current, 1.0)
                self._partial_close(pct_of_current, close)
            self.sl_price = self.entry_price
            logger.debug(f"  TP1: {self.symbol} LONG close {TP1_CLOSE_PCT*100:.0f}%, SL->breakeven")
        # TP2: +2.0xATR -> close 30%
        if self.trailing_step == 1 and atr_move >= TP2_ATR_MULT:
            self.trailing_step = 2
            if TP2_CLOSE_PCT > 0 and self.remaining_pct > 0:
                pct_of_current = TP2_CLOSE_PCT / self.remaining_pct
                pct_of_current = min(pct_of_current, 1.0)
                self._partial_close(pct_of_current, close)
            logger.debug(f"  TP2: {self.symbol} LONG close {TP2_CLOSE_PCT*100:.0f}%")
        # TSL update after TP1
        if self.trailing_step >= 1:
            new_sl = self._peak_price - ATR * TSL_ATR_LONG
    ...
    def _run_backtest(self, dfs, symbols, btc_df=None, btc_1d_df=None):
        ...
        for i, (timestamp, row) in enumerate(master_df.iterrows()):
            ...
            for sym in active_symbols:
                ...
                if atr_pct > 0.1:
                    sl_pct_long = SL_ATR_MULT_LONG * atr_pct / 100
                    sl_pct_short = 1.5 * atr_pct / 100
                    tp_pct = 2.0 * atr_pct / 100  # synced with strategist_client (was 3.0)
                else:
                    sl_pct_long = 0.03 * (SL_ATR_MULT_LONG / 1.5)
                    sl_pct_short = 0.03
                    tp_pct = 0.04  # proportional: 2.0/1.5 × 3% = 4%

                # ── LONG signal ──
                if composite > threshold_long and is_bullish and rsi < 70 and btc_1d_allows_long:
                    pending_signals[sym] = {"side": "long", "sl_pct": sl_pct_long, "tp_pct": tp_pct, "composite": composite, "atr": atr_value}

                # ── SHORT signal (with short limit check) ──
                elif composite < -threshold_short and not is_bullish and rsi > 30 and btc_1d_allows_short:
                    open_shorts = sum(1 for p in open_positions.values() if p.side == "short")
                    if open_shorts < MAX_SHORT_OPEN:
                        pending_signals[sym] = {"side": "short", "sl_pct": sl_pct_short, "tp_pct": tp_pct, "composite": composite, "atr": atr_value}
