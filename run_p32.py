#!/usr/bin/env python3
"""P32: Исправление LONG vs SHORT асимметрии.

Изменения vs P24:
1. composite_score передаётся в BacktestPosition (фикс бага)
2. LONG_TSL_FROM_ENTRY_ATR = 2.0 (было 1.5) — более широкий TSL для лонгов
"""
import sys, os, time
...
# P32: wider TSL for LONG entries
bt_mod.TSL_FROM_ENTRY_ATR = 2.0  # было 1.5
...
def run_one_period(pk, dfs, btc_df, btc_1d):
    ...
    for s in SYMBOLS:
        ...
        for t in res.trades:
            score = getattr(t, 'composite_score', 0.0)
...
# ── Run P32 ──
print(f"\n{'='*90}")
print(f"  P32: LONG_TSL_FROM_ENTRY_ATR = 2.0 (vs P24: 1.5)")
print(f"  composite_score fix: передаётся в BacktestPosition")
print(f"{'='*90}")

for pk, start_dt, end_dt in PERIODS:
    dfs, btc_df, btc_1d = data_cache[pk]
    r, long_scores, short_scores = run_one_period(pk, dfs, btc_df, btc_1d)
    p24 = P24[pk]
    if r is None:
        print(f"\n  {pk}: NO TRADES")
        continue
    ...
    print(f"  LONG  p25={percentile(long_scores,0.25):.3f} p50={percentile(long_scores,0.50):.3f} p75={percentile(long_scores,0.75):.3f} p90={percentile(long_scores,0.90):.3f}")
    print(f"  SHORT p25={percentile(short_scores,0.25):.3f} p50={percentile(short_scores,0.50):.3f} p75={percentile(short_scores,0.75):.3f} p90={percentile(short_scores,0.90):.3f}")
