#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P41: Verify SPOT-only backtest (LONG only, no shorts)."""
import sys, os, time
...
print("\nRunning SPOT backtest (LONG only, shorts disabled)...")
print("=" * 80)
...
for pk, start_dt, end_dt in PERIODS:
    ...
    print(f"  {pk:10s}  LONG: {len(long_t):>3} WR={long_wr:.1f}% PnL=${long_pnl:+.2f}  |  SHORT: {len(short_t):>3} PnL=${short_pnl:+.2f}  |  TOTAL: ${total_pnl:+.2f}  DD={max_dd:.1f}%")
    if long_t:
...
print(f"\n  GRAND TOTAL (SPOT LONG-only): ${grand_pnl:+.2f}  MaxDD={grand_dd:.1f}%")

# Also test at 2% risk for comparison
print("\n\n=== COMPARISON: 2% risk ===")
...
for pk, start_dt, end_dt in PERIODS:
    ...
    print(f"  {pk:10s}  LONG: ${long_pnl:+.2f}  SHORT: ${short_pnl:+.2f}  TOTAL: ${total_pnl:+.2f}  DD={max_dd:.1f}%")
    grand_pnl2 += total_pnl
print(f"  GRAND (2% risk): ${grand_pnl2:+.2f}")
