...
from risk_manager import RiskManager, PortfolioState, SizerConfig
...
GRANULARITY = "4H"  # основной ТФ для бэктеста
...
# ============================================================================
# Модифицированный Backtester с поддержкой двух режимов
# ============================================================================
class RegimeBacktester(Backtester):
    def __init__(self, symbol, candles_df, use_dynamic_threshold=True):
        super().__init__(symbol, candles_df)
        self.use_dynamic_threshold = use_dynamic_threshold
        self.regime_history = []  # список (timestamp, regime)
        self.trades_with_regime = []  # список (trade, regime)
        
    def _get_short_threshold(self, timestamp, regime=None) -> float:
        """Возвращает порог для SHORT в зависимости от режима."""
        if not self.use_dynamic_threshold:
            return 0.40  # старый flat порог
        
        if regime is None:
            # Определяем режим на момент timestamp
            # Для простоты берём последний известный режим
    ...
    def _regime_to_threshold(self, regime: str) -> float:
        if regime == "BEAR":
            return 0.30
        elif regime == "BULL":
            return 0.50
    ...
    async def run(self):
        """Запускает бэктест с отслеживанием режима."""
        # Предварительно вычисляем режим для каждой свечи
        for i in range(50, len(self.candles_df)):
            window = self.candles_df.iloc[:i+1]
...
# ============================================================================
async def compare_symbol(symbol: str) -> Dict:
    """Сравнивает два режима для одного символа."""
    print(f"  {symbol}: fetching candles...")
    df = fetch_candles(symbol, START_DATE, END_DATE, GRANULARITY)
    ...
    results = {
        "symbol": symbol,
        "old": {"trades": 0, "winrate": 0.0, "pnl_usd": 0.0, "pnl_pct": 0.0, "maxdd_pct": 0.0, "pf": 0.0, "trades_by_regime": {"BULL": 0, "BEAR": 0, "SIDEWAYS": 0}},
    ...
    # Определяем распределение режимов
    for i in range(50, len(df)):
        window = df.iloc[:i+1]
        regime = detect_regime_from_df(window)
        results["regime_distribution"][regime] += 1
    
    # Бэктест со старым порогом (0.40 flat)
    print(f"  {symbol}: old threshold (0.40 flat)...")
    bt_old = RegimeBacktester(symbol, df, use_dynamic_threshold=False)
    await bt_old.run()
    
    if bt_old.trades:
        wins = sum(1 for t in bt_old.trades if t.pnl_usd > 0)
        results["old"]["trades"] = len(bt_old.trades)
        results["old"]["winrate"] = wins / len(bt_old.trades) * 100 if bt_old.trades else 0
        results["old"]["pnl_usd"] = sum(t.pnl_usd for t in bt_old.trades)
        results["old"]["pnl_pct"] = results["old"]["pnl_usd"] / 10000 * 100
        results["old"]["maxdd_pct"] = bt_old.max_drawdown_pct
        all_wins = sum(t.pnl_usd for t in bt_old.trades if t.pnl_usd > 0)
        all_losses = sum(t.pnl_usd for t in bt_old.trades if t.pnl_usd < 0)
        results["old"]["pf"] = abs(all_wins / all_losses) if all_losses != 0 else (999 if all_wins > 0 else 0)
    
    # Бэктест с новым динамическим порогом
    print(f"  {symbol}: new dynamic threshold...")
    bt_new = RegimeBacktester(symbol, df, use_dynamic_threshold=True)
    await bt_new.run()
    
    if bt_new.trades_with_regime:
        results["new"]["trades"] = len(bt_new.trades_with_regime)
        wins = sum(1 for t, _ in bt_new.trades_with_regime if t.pnl_usd > 0)
        results["new"]["winrate"] = wins / len(bt_new.trades_with_regime) * 100 if bt_new.trades_with_regime else 0
        results["new"]["pnl_usd"] = sum(t.pnl_usd for t, _ in bt_new.trades_with_regime)
        results["new"]["pnl_pct"] = results["new"]["pnl_usd"] / 10000 * 100
        results["new"]["maxdd_pct"] = bt_new.max_drawdown_pct
        all_wins = sum(t.pnl_usd for t, _ in bt_new.trades_with_regime if t.pnl_usd > 0)
        all_losses = sum(t.pnl_usd for t, _ in bt_new.trades_with_regime if t.pnl_usd < 0)
        results["new"]["pf"] = abs(all_wins / all_losses) if all_losses != 0 else (999 if all_wins > 0 else 0)
...
async def main():
    print("=" * 100)
    print("СРАВНЕНИЕ ПОРОГОВ SHORT: старый (0.40 flat) vs новый динамический (BEAR=0.30, SIDEWAYS=0.40, BULL=0.50)")
    ...
    print("=" * 100)
    print(f"{'Symbol':<10} {'Regime B/E/S':<12} {'Trades O/N':<12} {'ΔTrades':<8} {'WinRate O/N':<14} {'PnL% O/N':<16} {'PF O/N':<12}")
    print("-" * 100)
    ...
    # Нужно больше данных для winrate по режимам — упростим
    ...
    # ============================================================================
    # Топ монет по приросту сделок
    # ============================================================================
    print("\n" + "=" * 100)
    print("ТОП МОНЕТ ПО ПРИРОСТУ СДЕЛОК (НОВЫЙ vs СТАРЫЙ)")
    print("=" * 100)
    sorted_by_delta = sorted(all_results, key=lambda x: x["new"]["trades"] - x["old"]["trades"], reverse=True)
    print(f"{'Symbol':<10} {'Trades O/N':<12} {'ΔTrades':<8} {'ΔWinRate%':<10} {'ΔPnL%':<10}")
    print("-" * 50)
    for res in sorted_by_delta[:5]:
        symbol = res["symbol"]
        delta = res["new"]["trades"] - res["old"]["trades"]
    ...
    print(f"ΔPnL%:                  {total_pnl_new - total_pnl_old:+.2f}%")
    ...
    with open("backtest_regime_comparison.json", "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)
    
    print(f"\nРезультаты сохранены в backtest_regime_comparison.json")
