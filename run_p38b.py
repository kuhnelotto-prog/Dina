...
print(f"\n  FULL TABLE:")
print(f"  SL  | BULL_PnL  SL% | BEAR_PnL  SL% | CUR_PnL  SL% |   GRAND")
baselines = [(4.1,-202.38,12.3,242.58,13.4,317.73,13.9),(4.2,-138.20,11.0,265.65,12.0,313.57,13.9),
             (4.3,-131.54,10.3,252.02,10.6,309.60,13.9),(4.4,-130.25,10.0,232.55,9.7,306.59,13.9),
             (4.5,-137.94,10.0,227.88,9.7,294.90,12.5)]
for sl, bp, bsl, bep, besl, cp, csl in baselines:
    print(f"  {sl:>3.1f} | {bp:>+9.2f} {bsl:>4.1f}% | {bep:>+9.2f} {besl:>4.1f}% | {cp:>+8.2f} {csl:>4.1f}% | {bp+bep+cp:>+8.2f}")
for sl_val in SWEEP_VALUES:
    r = results.get(sl_val, {}); b = r.get('BULL', {}); be = r.get('BEAR/SIDE', {}); c = r.get('CURRENT', {})
    bp = b.get('pnl',0); bsl = b.get('long_sl_rate',0); bep = be.get('pnl',0); besl = be.get('long_sl_rate',0)
    cp = c.get('pnl',0); csl = c.get('long_sl_rate',0)
    print(f"  {sl_val:>3.1f} | {bp:>+9.2f} {bsl:>4.1f}% | {bep:>+9.2f} {besl:>4.1f}% | {cp:>+8.2f} {csl:>4.1f}% | {bp+bep+cp:>+8.2f}")
