#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P24 Baseline Verification — matches run_p24.py exactly."""
import sys, os, time
...
print("Fetching P24 baseline data...")
data_cache = {}
...
def run_one_period(pk, dfs, btc_df, btc_1d):
    ...
    for s in SYMBOLS_12:
        ...
        if sym_df is None or sym_df.empty or len(sym_df) < 50:
            continue
        # KEY FIX: pass only current symbol + BTC (same as run_p24.py)
...
print(f"\n{'='*90}")
print(f"  P24 BASELINE VERIFICATION (dfs={{sym, BTC}} — matches run_p24.py)")
print(f"{'='*90}")
