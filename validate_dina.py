...
# Helper for running async code in Python 3.10+/3.14
...
def test_risk_manager():
    ...
    try:
        ...
        rm = RiskManager(sizer_config=SizerConfig(base_risk_pct=1.0), max_open_positions=1, daily_loss_limit=5.0, max_total_exposure_usd=50000.0)
        ...
        assert not status2.allowed, f"Expected BLOCKED, got: {status2.reason}"
        ...
        assert status3.allowed, f"Expected ALLOWED after close, got: {status3.reason}"
...
def test_executor():
    ...
    try:
        ...
        result = _run(executor.open_position(req))
...
def test_learning_engine():
    ...
    try:
        ...
        # Записываем сделки
        for _ in range(5):
            _run(engine.record_trade("trade1", ["rsi", "macd"], 1.5))
        for _ in range(3):
            _run(engine.record_trade("trade2", ["rsi"], -0.8))
        R.ok("record_trade() записал 8 сделок")

        weights = _run(engine.get_weights())
...
def test_signal_builder():
    ...
    try:
        ...
        _run(sb.update_candle("BTCUSDT", "1h", df))
        signal = _run(sb.compute("BTCUSDT"))
...
def test_attribution():
    ...
    try:
        ...
        # Запись закрытия
        _run(att.record_close("test1", exit_price=51000, pnl_pct=2.0, pnl_usd=200))
        R.ok("record_close() прошёл")

        # Отчёт
        report = _run(att.get_report(days=30))
...
def test_integration():
    ...
    try:
        ...
        sl = 49000
        confidence = 0.85
        status = _run(rm.check(portfolio, "BTCUSDT", entry, sl, confidence))
