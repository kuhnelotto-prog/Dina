#!/usr/bin/env python3
"""P38 CVD sweep: SL=6.6, MIN_PNL_TIMEOUT_LONG=24h, sizing fix.
Variants: CVD_WEIGHT_LONG=0.0, 0.1, 0.2, 0.3"""
...
def run_variant(label, cvd_weight):
    ...
    bt_mod.MIN_PNL_CHECK_H_LONG = 24  # P38: 24h timeout
...
for i, cvd_w in enumerate(CVD_VARIANTS):
    label = f"Baseline_CVD=0" if cvd_w == 0 else f"CVD={cvd_w}"
    print(f"Variant {i+1}/{len(CVD_VARIANTS)}: {label}")
    r = run_variant(label, cvd_w)
    results.append((cvd_w, r))

# Summary table
print("\n" + "="*110)
print("P38 CVD SWEEP: SL=6.6, MIN_PNL_TIMEOUT_LONG=24h, sizing fix, MIN_EXPECTED_PNL_PCT_LONG=-0.5%")
print("="*110)
hdr = f"{'Variant':28s} {'LONG#':>5s} {'WR%':>6s} {'PnL$':>10s} {'Timeout%':>8s} {'SL%':>5s} {'Total$':>10s} {'MaxDD%':>6s}"
print(hdr)
print("-"*110)
for cvd_w, r in results:
    label = f"P35_baseline_CVD=0" if cvd_w == 0 else f"P38_CVD={cvd_w}"
...
# Save
with open('p38_cvd_results.txt', 'w') as f:
    f.write("P38 CVD SWEEP: SL=6.6, MIN_PNL_TIMEOUT_LONG=24h, sizing fix\n")
    f.write(f"Fixed: SL_ATR_MULT_LONG=6.6, MIN_PNL_CHECK_H_LONG=24, MIN_EXPECTED_PNL_PCT_LONG=-0.5%\n")
    f.write("="*110 + "\n")
    f.write(hdr + "\n")
    f.write("-"*110 + "\n")
    for cvd_w, r in results:
    ...
    f.write("="*110 + "\n")
print("\nResults written to p38_cvd_results.txt")