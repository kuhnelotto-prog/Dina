...
# COMPLETE TABLE
print(f"\n{'='*90}")
print(f"  FINE-TUNE AROUND SL=6.5 COMPLETE TABLE")
print(f"{'='*90}")
...
all_data = [
    ...
    (6.2, -17.63, 4.7, 277.66, 3.2, 330.48, 2.8),
    (6.4, 37.61, 3.7, 278.63, 3.2, 329.78, 2.8),
    (6.5, 35.79, 3.7, 279.10, 3.2, 329.44, 2.8),
    (6.6, 46.28, 3.3, 281.20, 2.8, 329.06, 2.8),
    (7.0, 9.04, 3.3, 283.91, 2.8, 329.06, 1.4),
]
for sl, bp, bsl, bep, besl, cp, csl in all_data:
    grand = bp + bep + cp
    marker = " <--- BEST" if sl == 6.6 else ""
...
for sl_val in SWEEP_VALUES:
    ...
    grand = bp + bep + cp
    marker = " <--- NEW BEST?" if grand > 656.54 else ""
