#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P35 Sweep: SL_ATR_MULT_LONG = 3.3, 3.4, 3.6."""
import sys, os, time
...
SWEEP_VALUES = [3.3, 3.4, 3.6]

print("Fetching data (shared across all sweep values)...")
data_cache = {}
...
def run_one_period(pk, dfs, btc_df, btc_1d, sl_mult):
    ...
    for s in SYMBOLS_12:
        sym_df = dfs.get(s)
        if sym_df is None or sym_df.empty or len(sym_df) < 50:
            continue
    ...
    return {
        ...
        "short_sl_rate": short_sl/len(short_t)*100 if short_t else 0,
    }
...
print(f"{'='*90}")
header = f"{'SL':>4s} | {'BULL_PnL':>9s} {'BULL_WR':>7s} {'BULL_SL':>7s} | {'BEAR_PnL':>9s} {'BEAR_WR':>7s} {'BEAR_SL':>7s} | {'CUR_PnL':>8s} {'CUR_WR':>7s} {'CUR_SL':>7s} | {'GRAND':>8s}"
print(header)
print("-"*len(header))
# Add P24 and P34 baselines
baselines = [
    (1.5, -652.86, 37.8, 39.6, -219.96, 35.7, 40.6, 432.80, 42.2, 36.7),
    (3.2, -443.42, 56.2, 22.4, 189.28, 57.3, 22.0, 400.36, 58.3, 20.8),
    (3.5, -299.53, 57.1, 17.5, 175.76, 57.3, 21.1, 340.53, 58.3, 19.4),
]
for sl, bp, bwr, bsl, bep, bewr, besl, cp, cwr, csl in baselines:
    grand = bp + bep + cp
    print(f"  {sl:>3.1f} | {bp:>+9.2f} {bwr:>6.1f}% {bsl:>6.1f}% | {bep:>+9.2f} {bewr:>6.1f}% {besl:>6.1f}% | {cp:>+8.2f} {cwr:>6.1f}% {csl:>6.1f}% | {grand:>+8.2f}")
...
for sl_val in SWEEP_VALUES:
    ...
    be = r.get('BEAR/SIDE', {})
    c = r.get('CURRENT', {})
    bp = b.get('pnl', 0); bwr = b.get('long_wr', 0); bsl = b.get('long_sl_rate', 0)
    bep = be.get('pnl', 0); bewr = be.get('long_wr', 0); besl = be.get('long_sl_rate', 0)
    cp = c.get('pnl', 0); cwr = c.get('long_wr', 0); csl = c.get('long_sl_rate', 0)
    grand = bp + bep + cp
    print(f"  {sl_val:>3.1f} | {bp:>+9.2f} {bwr:>6.1f}% {bsl:>6.1f}% | {bep:>+9.2f} {bewr:>6.1f}% {besl:>6.1f}% | {cp:>+8.2f} {cwr:>6.1f}% {csl:>6.1f}% | {grand:>+8.2f}")
